...
test("Director scroll fix never hijacks wheel default behavior", () => {
  assert.ok(!scrollFix.includes("preventDefault("), "scroll compatibility must not prevent native wheel scrolling");
...
test("Studio boundary only blocks LiteGraph wheel propagation", () => {
  ...
  assert.ok(!studio.match(/root\.addEventListener\("wheel"[\s\S]{0,180}preventDefault/));
});
